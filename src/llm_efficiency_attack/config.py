...
# control, not as a feature: it spends the identical perturbation budget while
# ignoring the gradient, so comparing the two is what shows whether the
# white-box signal is actually doing any work.
...
class AttackConfig:
    ...
    Attributes:
        ...
        perturbation_budget: Maximum number of *distinct token positions* that
            may differ between `x` and `adv_x`. This is the imperceptibility
            constraint.
        top_k: How many candidate substitutions, ranked by the first-order
            estimate, get an exact forward pass. The linear approximation is
            reliable for ranking but not for magnitude, so the shortlist is
            re-scored exactly before anything is committed.
        ...
        objective_horizon: How many decoder steps the objective looks at. One
            step asks only "will you stop immediately?"; a longer horizon asks
            the model to keep declining to stop, which is a stronger signal.
        seed: Seeds Python, NumPy and torch RNGs so a run is reproducible.
        device: Torch device string. `"auto"` picks CUDA when available.
